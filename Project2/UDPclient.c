...
int createSocket(char *serverName, int serverPort, struct sockaddr_in * dest)
{
    ...
    */
    ...
    dest->sin_port = htons((uint16_t)serverPort);//Fills in the port number (host to network short)
...
int sendRequest(int sock, char *request, struct sockaddr_in *dest){
    ...
    if(sendto(sock, request, strlen(request), 0, (const struct sockaddr *) dest, sizeof(struct sockaddr_in)) < 0){
        perror("\nError: Send failure");
        return -1;
    }
    return 0;
